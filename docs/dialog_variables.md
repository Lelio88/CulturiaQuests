...
| **`[DungeonThreshold]`** | Le palier du bastion où se déroule l'objectif. | `[DungeonThreshold]` → **"3"** | Utilisé pour indiquer la localisation précise d'un objet ou d'une cible dans les quêtes de type "Musée". |
...
function formatDialog(rawText: string, context: { playerName: string; dungeonLevel?: number }): string {
  ...
  // Note : dungeonLevel peut être null si la quête ne se déroule pas dans un bastion
  if (context.dungeonLevel !== undefined) {
    formattedText = formattedText.replaceAll('[DungeonThreshold]', context.dungeonLevel.toString());
  } else {
    // Fallback si le palier n'est pas connu (optionnel)
...
**Entrée (BDD) :**
> "Je crois qu’elle s’est cachée au palier **[DungeonThreshold]**… tu pourrais m’aider à la retrouver, **[PlayerName]** ?"
...
**Sortie (Écran) :**
> "Je crois qu’elle s’est cachée au palier **5**… tu pourrais m’aider à la retrouver, **Link** ?"
